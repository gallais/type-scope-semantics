...
record Synchronisable
  ...
  field
    ...
    R⟦var⟧    :  {Γ Δ : Context} {σ : Type} {ρ^A : Var Γ ⇒[ 𝓔^A ] Δ} {ρ^B : Var Γ ⇒[ 𝓔^B ] Δ}
                 ...
                 --------------------------------------------------------------
                   𝓡 (𝓢^A.⟦var⟧ (lookup ρ^A v)) (𝓢^B.⟦var⟧ (lookup ρ^B v))

    R⟦λ⟧      :  {Γ : Context} {σ τ : Type}
                 {f^A : Kripke 𝓔^A 𝓜^A Γ σ τ} {f^B : Kripke 𝓔^B 𝓜^B Γ σ τ} →

                     RKripke 𝓔^R 𝓜^R f^A f^B →
                 -----------------------------------------------------------------------
                     𝓡 (𝓢^A.⟦λ⟧ f^A) (𝓢^B.⟦λ⟧ f^B)

    R⟦$⟧      :  RApplicative 𝓢^A._⟦$⟧_ 𝓢^B._⟦$⟧_ 𝓜^R
    ...
    R⟦ifte⟧   :  {Γ : Context} {σ : Type}
                 ...
                 {l^A r^A : 𝓜^A Γ σ} {l^B r^B : 𝓜^B Γ σ} →
                   ...
                   𝓡 b^A b^B → related 𝓜^R l^A l^B → related 𝓜^R r^A r^B →
                 ----------------------------------------------------------
                   𝓡 (𝓢^A.⟦ifte⟧ b^A l^A r^A) (𝓢^B.⟦ifte⟧ b^B l^B r^B)
...
module Fundamental
  ...
  {𝓢^A  : Semantics 𝓔^A 𝓜^A} {𝓢^B : Semantics 𝓔^B 𝓜^B}
  {𝓔^R  : RModel ℓ^RE 𝓔^A 𝓔^B} {𝓜^R : RModel ℓ^RM 𝓜^A 𝓜^B}
  (𝓢^R : Synchronisable 𝓢^A 𝓢^B 𝓔^R 𝓜^R)
  where

  open Synchronisable 𝓢^R
  ...
  lemma :  {Γ Δ : Context} {σ : Type} (t : Γ ⊢ σ)
           ...
           𝓡 (eval 𝓢^A t ρ^A) (eval 𝓢^B t ρ^B)
